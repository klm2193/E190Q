...
{   
    ...
    public class Navigation
    {
        ...
        public int numParticles = 7000; //5000
        ...
        private int laserStepSize = 30; //18
        ...
        int fastSize = 200; //200;
        int slowSize = 1000;
        double alphaSlow = 0.02;
        double alphaFast = 10; //10;
        ...
        public void LocalizeEstWithParticleFilter()
        {
            ...
            if (newLaserData)
            {
                ...
                for (int i = 0; i < numParticles; i++)
                {
                    ...
                    avgWeight += propagatedParticles[i].w / (double)numParticles;
                }
                newLaserData = false;
            }
            ...
            /*
            double rand = random.NextDouble() * (1.0/(double)numParticles);
            double c = propagatedParticles[0].w;
            int k = 0;
            int count = 0;
            for (int m = 0; m < numParticles; m++)
            {
                double U = rand + m * (1.0 / (double)numParticles);

                while (U > c) 
                {
                    k = k + 1;
                    c = c + propagatedParticles[k].w;
                }

                particles[count] = propagatedParticles[k];


                totalX = particles[count].x + totalX;
                totalY = particles[count].y + totalY;

                TReal = Math.Cos(particles[count].t);
                TImag = Math.Sin(particles[count].t);
            ...
            slowQueue.Enqueue(avgWeight);
            fastQueue.Enqueue(avgWeight);

            wSlow = (avgWeight - slowQueue.Peek()) / slowSize;
            wFast = (avgWeight - fastQueue.Peek()) / fastSize;

            wSlow += alphaSlow * (avgWeight - wSlow);
            wFast += alphaFast * (avgWeight - wFast);

            double randomProb = 1 - wFast / wSlow;

            //randomProb = Math.Min(0.5, Math.Max(0, randomProb));
            randomProb = Math.Max(0, randomProb);
            ...
            for (int m = 0; m < numParticles; m++)
            {
                ...
                if ((randChosen <= randomProb) && (particles[m].w <= 0.15))
                {
                    SetRandomPos(m);
                    //CalculateWeight(m);
                }
        ...
        void CalculateWeight(int p)
        {
            ...
            for (int i = 0; i < LaserData.Length; i = i + 3*laserStepSize)
            {
