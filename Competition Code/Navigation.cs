...
namespace DrRobot.JaguarControl
{
    ...
    public class Navigation
    {
        ...
        public double wheelRadius = 0.105;//0.089;
        public double robotRadius = 0.2225;//0.242;//0.232
        ...
        double gamma1 = 0.0902;
        double gamma2 = 1.5163;
        ...
        double phi11_L = 0.8088;
        double phi12_L = 0.0449;
        double phi21_L = -2.8757;
        double phi22_L = 0.0899;

        double gamma1_L = 0.1912;
        double gamma2_L = 2.8757;
        ...
        // chris and robert variables
        double decay_rate = 0.5;
        public double u_L_curr = 0;
        public double u_R_curr = 0;
        public double u_L_avg = 0;
        public double u_R_avg = 0;
        public System.Collections.Queue pastValsL = new System.Collections.Queue();
        public System.Collections.Queue pastValsR = new System.Collections.Queue();
        double[] weights = new double[] { 0.0425, 0.0675, 0.13, 0.255, 0.505 };
        ...
        public void Initialize()
        {
            ...
            initialX = -3; // Defining the initial values
            initialY = -8.125;
            initialT = 0;
        ...
        public void UpdateSensorMeasurements()
        {
            ...
            if (jaguarControl.Simulating())
            {
                ...
                if (laserCounter >= 500)//2000)
                {
                    ...
                    {
                        LaserData[i] = (long)(1000 * map.GetClosestWallDistanceSimRobot(x, y, t - 1.57 + laserAngles[i]));
                    }
                    laserCounter = 0;
                    newLaserData = true;
        ...
        public void CalcMotorSignalsCR()
        {
            ...
            double maxIntErr = 10000 / deltaT;
            double maxDiffErr = 10000 * deltaT;

            e_L = desiredRotRateL - diffEncoderPulseL / deltaT;
            e_R = desiredRotRateR - diffEncoderPulseR / deltaT;

            e_sum_L = decay_rate * e_sum_L + e_L * deltaT;
            e_sum_R = decay_rate * e_sum_R + e_R * deltaT;

            e_sum_L = Math.Max(-maxIntErr, Math.Min(e_sum_L, maxIntErr));
            e_sum_R = Math.Max(-maxIntErr, Math.Min(e_sum_R, maxIntErr));

            double e_diff_L = (e_L - e_L_last) / deltaT;
            double e_diff_R = (e_R - e_R_last) / deltaT;
            e_diff_L = Math.Max(-maxDiffErr, Math.Min(e_diff_L, maxDiffErr));
            e_diff_R = Math.Max(-maxDiffErr, Math.Min(e_diff_R, maxDiffErr));
            e_L_last = e_L;
            e_R_last = e_R;

            u_L_curr = ((K_p * e_L) + (K_i * e_sum_L) + (K_d * e_diff_L));

            u_R_curr = ((K_p * e_R) + (K_i * e_sum_R) + (K_d * e_diff_R));
            // The following settings are used to help develop the controller in simulation.
            // They will be replaced when the actual jaguar is used.
            //motorSignalL = (short)(zeroOutput + desiredRotRateL * 100);// (zeroOutput + u_L);
            //motorSignalR = (short)(zeroOutput - desiredRotRateR * 100);//(zeroOutput - u_R);

            // smooth control signals with moving average
            pastValsL.Enqueue(u_L_curr);
            pastValsR.Enqueue(u_R_curr);

            pastValsL.Dequeue();
            pastValsR.Dequeue();

            System.Collections.IEnumerator valEnumL = pastValsL.GetEnumerator();
            System.Collections.IEnumerator valEnumR = pastValsR.GetEnumerator();

            int w = 0;
            u_L_avg = 0;
            u_R_avg = 0;

            while (valEnumL.MoveNext())
            {
                valEnumR.MoveNext();
                double currentL = (double)valEnumL.Current;
                double currentR = (double)valEnumR.Current;
                u_L_avg += currentL * weights[w];
                u_R_avg += currentR * weights[w];
                w++;
            }

            motorSignalL = (short)(zeroOutput + u_L_avg);
            motorSignalR = (short)(zeroOutput - u_R_avg);
        ...
        public void KalmanEstimator()
        {
            short zeroOutput = 16383;       //sets lower limit to motor signal
            short maxPosOutput = 32767;     //sets upper limit to motor signal

            double rotRateL = diffEncoderPulseL / deltaT;
            double rotRateR = diffEncoderPulseR / deltaT;

            // calculate the new state variables
            double x1nL = L1_L * rotRateL + x1storeL;
            double x2nL = L2_L * rotRateL + x2storeL;

            double x1nR = L1 * rotRateR + x1storeR;
            double x2nR = L2 * rotRateR + x2storeR;

            // calculate the new input signal
            unL = -k1_L * x1nL - k2_L * x2nL + N_L * desiredRotRateL + uinL;

            unR = -k1 * x1nR - k2 * x2nR + N * desiredRotRateR + uinR;

            // limit the output velocity

            // update uin and xstore things
            uinL = uinL - Ki_L * rotRateL + Ki_L * desiredRotRateL;
            x1storeL = (phi11_L - L1_L) * x1nL + phi12_L * x2nL + gamma1_L * unL;
            x2storeL = (phi21_L - L2_L) * x1nL + phi22_L * x2nL + gamma2_L * unL;

            uinR = uinR - Ki * rotRateR + Ki * desiredRotRateR;
            x1storeR = (phi11 - L1) * x1nR + phi12 * x2nR + gamma1 * unR;
            x2storeR = (phi21 - L2) * x1nR + phi22 * x2nR + gamma2 * unR;


            motorSignalL = (short)(zeroOutput + unL);
            motorSignalR = (short)(zeroOutput - unR);
        ...
        public void TurnLoggingOn()
        {
            ...
            streamPath_ = "../../../Data/JaguarData_" + date + "LEFT" + ".csv";
            logFile = File.CreateText(streamPath_);
            //string header = "time,realx,realy,realt, predx,predy,predt,sdx,sdy,sdt";
            string header = "time,motorSignalL,motorSignalR,diffEncoderPulseL,diffEncoderPulseR,desiredRotRateL,desiredRotRateL";
        ...
        private void LogData()
        {
            if (loggingOn)
            {
                ...
                String newData = time.ToString() + "," + motorSignalL.ToString() + "," + motorSignalR.ToString() + "," + diffEncoderPulseL.ToString() +
                    "," + diffEncoderPulseR.ToString() + "," + actL.ToString() + "," + actR.ToString();
        ...
        private void newTrackTrajectory()
        {
            ...
            double distAhead = 3.0;



            // path 1: straight line east from M0
            if (pathIndex == 1)
            {
                double startY = -8.1250;
                double startT = 0.0;
            ...
            // path 2: curve (clockwise) between M0 and M1
            if (pathIndex == 2)
            {
                ...
                else
                {
                    ...
                    //desiredX = circleRad * Math.Cos(desiredAngle);  //THIS MIGHT BE WRONG< CHECK IT. OR DESIRED ANGLE
                    //desiredY = circleRad * Math.Sin(desiredAngle);
            ...
            // path 3: straight line south from M1 toward M2
            if (pathIndex == 3)
            {
                double startX = 0.0;
                double goalY = -14.25;
                double startT = -(Math.PI / 2.0);
                ...
                {
                    desiredX = startX;
                    desiredY = y_est - distAhead;
                    desiredT = startT;
                }
            }

            // path 4: curve between M1 and M2 (counterclockwise)
            if (pathIndex == 4)
            {
            ...
            // path 5: curve between M1 and M2 (clockwise)
            if (pathIndex == 5)
            {
            ...
            // path 7: curve between M2 and M4 (counterclockwise)
            if (pathIndex == 7)
            {
            ...
            // path 8: curve between M2 and M4 (clockwise)
            if (pathIndex == 8)
            {
            ...
            // path 9: straight line south from M2 to M4
            if (pathIndex == 9)
            {
                double startX = 4.0;
                double goalY = -27.5;
            ...
            // path 10: straight line north (robot should back up) from M2
            if (pathIndex == 10)
            {
                double startX = 4.0;
                double goalY = -25.5;
            ...
            // path 11: curve between M4 and M3
            if (pathIndex == 11)
            {
            ...
            // path 12: straight line west from M4
            if (pathIndex == 12)
            {
                double startY = -25.5;
                double startT = Math.PI;
            ...
            // path 13: curve toward M3 (counterclockwise)
            if (pathIndex == 13)
            {
            ...
            // path 14: straight line south through M3
            if (pathIndex == 14)
            {
                double startX = 0.0;
                double goalY = -40.0;
